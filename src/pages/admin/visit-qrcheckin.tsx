...
const VisitCheckinAdmin: React.FC = () => {
  ...
  const checkinsToday = visits.filter(
    (v) => v.status === "checked-in" && v.date === "2025-02-18"
  ).length;
  ...
  const generateQR = async (visit: Visit) => {
    ...
    try {
      ...
      setVisits((prev) =>
        prev.map((v) => (v.id === visit.id ? { ...v, qrCode: qrBase64 } : v))
      );
    } catch (err) {
  ...
  const handleCheckin = () => {
    ...
    if (visit && visit.status === "approved") {
      setVisits((prev) =>
        prev.map((v) =>
          v.id === visit.id ? { ...v, status: "checked-in" as const } : v
        )
      );
      setCheckinAlert({
        type: "success",
        message: `Checked in: ${visit.visitorName} visiting ${visit.residentName}`,
      });
    } else {
      setCheckinAlert({
        type: "error",
        message: "Invalid QR code or visit not approved",
      });
    }
    setQrInput("");
  ...
  return (
    <div className="container mx-auto p-6 space-y-6 min-h-screen">
      <h1 className="text-3xl font-bold text-blue-800">Visit & QR Check-in</h1>
        ...
        <Tabs defaultValue="visits" className="w-full max-w-[1100px]">
          ...
          <TabsContent value="visits">
            ...
            <Card className="shadow-sm">
              <CardHeader>
                <CardTitle className="text-xl">Visit Records</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
